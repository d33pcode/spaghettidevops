...
**inductive step** \
Assuming $t(n) = 2^{n-1}$, show that $t(n+1) = 2^{n}$: $$t(n+1) = t(n) + t(n-1) + t(n-2) + t(n-3) + ... + t(1) +1$$ we can easily see that, aside from the first term, the recurrence equation is the same as before, so we can substitute: $$t(n + 1)= t(n) + t(n) = 2^{n-1} + 2^{n-1} = 2^{n}$$
